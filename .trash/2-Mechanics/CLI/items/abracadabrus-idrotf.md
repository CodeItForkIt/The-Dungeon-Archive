...
The chest regains `dice:1d20|noform|avg` (``) expended charges daily at dawn. If the item's last charge is expended, roll a `dice:d20|noform|avg` (`d20`). On a 1, the chest loses its magic (becoming an ordinary chest), and its gemstones turn to dust.
