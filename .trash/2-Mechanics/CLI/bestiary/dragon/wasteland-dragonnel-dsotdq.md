...
***Rend.*** *Melee Weapon Attack:* `dice:1d20+5|noform|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+3|noform|avg|text(10)` (`2d6 + 3`) slashing damage.

***Acid Spit.*** *Ranged Weapon Attack:* `dice:1d20+5|noform|text(+5)` to hit, range 60 ft., one target. *Hit:* `dice:5d6+3|noform|avg|text(20)` (`5d6 + 3`) acid damage.
```
^statblock