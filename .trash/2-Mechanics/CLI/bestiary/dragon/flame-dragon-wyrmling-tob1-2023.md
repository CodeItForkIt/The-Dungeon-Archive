...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/ToB1-2023/Flame%20Dragon%20Wyrmling.webp#token)
...
***Heated Body.*** A creature that touches the flame dragon or hits it with a melee attack while within 5 feet of it takes `dice:1d6|noform|avg|text(3)` (`1d6`) fire damage.

## Actions

***Bite.*** *Melee Weapon Attack:* `dice:1d20+3|noform|text(+3)` to hit, reach 5 ft., one target. *Hit:* `dice:1d10+1|noform|avg|text(6)` (`1d10 + 1`) piercing damage plus `dice:1d6|noform|avg|text(3)` (`1d6`) fire damage.

***Fire Breath (Recharge 5-6).*** The dragon exhales fire in a 15-foot cone. Each creature in that area must make a DC 12 Dexterity saving throw, taking `dice:5d6|noform|avg|text(17)` (`5d6`) fire damage on a failed save, or half as much damage on a successful one.
```
^statblock
