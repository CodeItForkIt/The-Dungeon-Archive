...
- **Languages** understands the languages that [Tary](2-Mechanics/CLI/bestiary/humanoid/taryon-darrington-tdcsr.md) speaks
- **Challenge** 6

## Traits

***Arcane Surge (4/Day).*** When Doty hits a target, [Tary](2-Mechanics/CLI/bestiary/humanoid/taryon-darrington-tdcsr.md) can channel magical energy to deal an extra `dice:4d6|noform|avg|text(14)` (`4d6`) force damage to the target as long as [Tary](2-Mechanics/CLI/bestiary/humanoid/taryon-darrington-tdcsr.md) is not [incapacitated](2-Mechanics/CLI/rules/conditions.md#Incapacitated).
...
***Empowered Fist.*** *Melee Weapon Attack:* `dice:1d20+7|noform|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+4|noform|avg|text(8)` (`1d8 + 4`) force damage.

***Repair (3/Day).*** Doty regains `dice:2d8+6|noform|avg|text(15)` (`2d8 + 6`) hit points.

## Reactions

***Deflect.*** If a creature within 5 feet of Doty that he can see makes an attack against a creature other than Doty, he can impose disadvantage on the attack roll. The attacker then takes `dice:1d4+4|noform|avg|text(6)` (`1d4 + 4`) force damage.
```
^statblock