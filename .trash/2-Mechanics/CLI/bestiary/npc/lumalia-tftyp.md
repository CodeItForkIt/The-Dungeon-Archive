...
***Angelic Weapons.*** Lumalia's weapon attacks are magical. When Lumalia hits with any weapon, the weapon deals an extra `dice:4d8|noform|avg` (`4d8`) radiant damage (included in the attack).
...
***Healing Touch (3/Day).*** Lumalia touches another creature. The target magically regains `dice:4d8+2|noform|avg|text(20)` (`4d8 + 2`) hit points and is freed from any curse, disease, poison, blindness, or deafness.
