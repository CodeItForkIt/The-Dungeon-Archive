...
***Divine Rejuvenation.*** When Auril drops to 0 hit points, her body turns to slush and melts away. Auril instantly reappears in her [second form](2-Mechanics/CLI/bestiary/npc/auril-second-form-idrotf.md), in an unoccupied space within 60 feet of where her first form disappeared. Her initiative count doesn't change.
...
***Innate Spellcasting.*** Auril's innate spellcasting ability is Charisma (spell save DC 21, `dice:1d20+13|noform|text(+13)` to hit with spell attacks). She can innately cast the following spells, requiring no material components:

**At will**: [chromatic orb](2-Mechanics/CLI/spells/chromatic-orb.md) (cold orb only; see "Actions" below), [detect magic](2-Mechanics/CLI/spells/detect-magic.md), [misty step](2-Mechanics/CLI/spells/misty-step.md)

**2/day each**: [control weather](2-Mechanics/CLI/spells/control-weather.md), [detect thoughts](2-Mechanics/CLI/spells/detect-thoughts.md), [ice storm](2-Mechanics/CLI/spells/ice-storm.md)

## Actions

***Multiattack.*** Auril attacks twice with her talons.

***Talons.*** *Melee Weapon Attack:* `dice:1d20+7|noform|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+3|noform|avg|text(7)` (`1d8 + 3`) piercing damage plus `dice:1d6|noform|avg|text(3)` (`1d6`) cold damage.

***Touch of Frost.*** *Melee Spell Attack:* `dice:1d20+13|noform|text(+13)` to hit, reach 5 ft., one creature. *Hit:* `dice:3d8|noform|avg|text(13)` (`3d8`) cold damage, and the target can't take reactions until the start of its next turn.

***Chromatic Orb.*** *Ranged Spell Attack:* `dice:1d20+13|noform|text(+13)` to hit, range 90 ft., one creature. *Hit:* `dice:3d8|noform|avg|text(13)` (`3d8`) cold damage.
