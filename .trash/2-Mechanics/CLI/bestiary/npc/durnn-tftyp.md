...
***Martial Advantage.*** Once per turn, the hobgoblin can deal an extra `dice:2d6|noform|avg|text(7)` (`2d6`) damage to a creature it hits with a weapon attack if that creature is within 5 feet of an ally of the hobgoblin that isn't [incapacitated](2-Mechanics/CLI/rules/conditions.md#Incapacitated).

## Actions

***Longsword.*** *Melee Weapon Attack:* `dice:1d20+4|noform|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+1|noform|avg|text(5)` (`1d8 + 1`) slashing damage, or `dice:1d10+2|noform|avg|text(6)` (`1d10 + 2`) slashing damage if used with two hands.
