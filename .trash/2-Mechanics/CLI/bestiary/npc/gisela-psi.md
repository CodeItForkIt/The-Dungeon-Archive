...
```ad-statblock
title: Gisela
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/PSI/Gisela.webp#token)
...
***Angelic Weapons.*** The Gisela's weapon attacks are magical. When the Gisela hits with any weapon, the weapon deals an extra `dice:5d8|noform|avg` (`5d8`) radiant damage (included in the attack).
...
***Healing Touch (4/Day).*** The Gisela touches another creature. The target magically regains `dice:6d8+3|noform|avg|text(30)` (`6d8 + 3`) hit points and is freed from any curse, disease, poison, blindness, or deafness.
```
^statblock