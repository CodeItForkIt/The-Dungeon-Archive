...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/WDMM/Thwad%20Underbrew.webp#token)
...
***Maul.*** *Melee Weapon Attack:* `dice:1d20+9|noform|text(+9)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+5|noform|avg|text(12)` (`2d6 + 5`) bludgeoning damage, plus `dice:2d6|noform|avg|text(7)` (`2d6`) bludgeoning damage if Thwad has more than half of his total hit points remaining.

***Shortbow.*** *Ranged Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, range 80/320 ft., one target. *Hit:* `dice:1d6+2|noform|avg|text(5)` (`1d6 + 2`) piercing damage, plus `dice:2d6|noform|avg|text(7)` (`2d6`) piercing damage if Thwad has more than half of its total hit points remaining.
```
^statblock