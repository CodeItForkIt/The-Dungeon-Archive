---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/coa
- monster/cr/17
- monster/size/large
- monster/type/fey
aliases: ["Anacreda"]
---
# Anacreda
*Source: Chains of Asmodeus p. 146*  

```ad-statblock
title: Anacreda
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/CoA/Anacreda.webp#token)
*Large fey, Neutral Evil*

- **Armor Class** 19 (natural armor)
- **Hit Points** 253 (`22d10 + 132`)
- **Speed** 40 ft., fly 40 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|25 (+7)|16 (+3)|22 (+6)|15 (+2)|16 (+3)|19 (+4)|

- **Proficiency Bonus** +6
- **Saving Throws** Constitution +12, Charisma +10
- **Skills** Deception +10, Perception +9
- **Senses** darkvision 120 ft., truesight 10 ft., passive Perception 19
- **Damage Immunities** cold; fire; bludgeoning, piercing, slashing from nonmagical attacks
- **Languages** Common, Giant, Infernal, Sylvan
- **Challenge** 17

## Actions

***Multiattack.*** Anacreda makes two Claw attacks. She can replace one of the attacks with a Crushing Hug attack.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+13|noform|text(+13)` to hit, reach 5 ft., one target. *Hit:* `dice:2d8+7|noform|avg|text(16)` (`2d8 + 7`) slashing damage.

***Crushing Hug.*** *Melee Weapon Attack:* `dice:1d20+13|noform|text(+13)` to hit, reach 5 ft., one target. *Hit:* `dice:8d10+7|noform|avg|text(51)` (`8d10 + 7`) bludgeoning damage, and if the target is a Large or smaller creature, Anacreda grabs it with both arms. The target has the [grappled](2-Mechanics/CLI/rules/conditions.md#Grappled) condition (escape DC 18). Until the grapple ends, the target takes `dice:12d10+7|noform|avg|text(73)` (`12d10 + 7`) necrotic damage at the start of each of Anacreda's turns. Anacreda can't make attacks while grappling a creature in this way.

***Spinewall.*** Anacreda causes a 50-foot-long, 5-foot-wide row of bones to pierce the earth at a point within 120 feet of her. She chooses the direction of the line. The bones rise 10 feet into the air and form a solid wall. A 5-foot section of bone has an AC of 20 and 10 hit points.

***Shattering Shriek (1/Day).*** Anacreda shrieks loud enough to shatter stone and bone alike. Each creature within a 50-foot-radius sphere centered on Anacreda must make a DC 18 Constitution saving throw. A creature takes `dice:10d10|noform|avg|text(55)` (`10d10`) thunder damage on a failed save, or half as much damage on a successful one. Unworked stone, exposed bone, and similar materials in the radius are shattered and turned into difficult terrain.
```
^statblock