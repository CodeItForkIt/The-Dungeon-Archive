...
```ad-statblock
title: Ignia
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/DSotDQ/Ignia.webp#token)
...
***Multiattack.*** Ignia makes three attacks: one with its bite and two with its claws.
...
***Fire Breath (Recharge 5-6).*** Ignia exhales fire in a 30-foot cone. Each creature in that area must make a DC 17 Dexterity saving throw, taking `dice:16d6|noform|avg|text(56)` (`16d6`) fire damage on a failed save, or half as much damage on a successful one.
```
^statblock