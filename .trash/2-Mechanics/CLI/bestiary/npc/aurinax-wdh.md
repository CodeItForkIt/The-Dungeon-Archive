...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+14|noform|text(+14)` to hit, reach 10 ft., one target. *Hit:* `dice:2d10+8|noform|avg|text(19)` (`2d10 + 8`) piercing damage.
...
- **Fire Breath.** Aurinax exhales fire in a 60-foot cone. Each creature in that area must make a DC 21 Dexterity saving throw, taking `dice:12d10|noform|avg|text(66)` (`12d10`) fire damage on a failed save, or half as much damage on a successful one.  
...
***Detect.*** Aurinax makes a Wisdom ([Perception](2-Mechanics/CLI/rules/skills.md#Perception)) check.

***Tail Attack.*** Aurinax makes a tail attack.

***Wing Attack (Costs 2 Actions).*** Aurinax beats its wings. Each creature within 10 feet of Aurinax must succeed on a DC 22 Dexterity saving throw or take `dice:2d6+8|noform|avg|text(15)` (`2d6 + 8`) bludgeoning damage and be knocked [prone](2-Mechanics/CLI/rules/conditions.md#Prone). Aurinax can then fly up to half its flying speed.
```
^statblock