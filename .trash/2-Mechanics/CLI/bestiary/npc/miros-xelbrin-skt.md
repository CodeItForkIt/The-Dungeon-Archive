...
```ad-statblock
title: Miros Xelbrin
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/SKT/Miros%20Xelbrin.webp#token)
...
***Bear Hug.*** *Melee Weapon Attack:* `dice:1d20+5|noform|text(+5)` to hit, reach 5 ft., one creature. *Hit:* `dice:1d4+3|noform|avg|text(5)` (`1d4 + 3`) bludgeoning damage, and the target [grappled](2-Mechanics/CLI/rules/conditions.md#Grappled) (escape DC 13) and takes `dice:1d4+3|noform|avg|text(5)` (`1d4 + 3`) bludgeoning damage at the start of each of Miros's turns until the grapple ends. Miros cannot make attacks while grappling a creature.

***Club.*** *Melee Weapon Attack:* `dice:1d20+5|noform|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+1|noform|avg|text(5)` (`1d4 + 1`) bludgeoning damage

***Heavy Crossbow.*** *Ranged Weapon Attack:* `dice:1d20+2|noform|text(+2)` to hit, range 100/400 ft., one target. *Hit:* `dice:1d10|noform|avg|text(5)` (`1d10`) piercing damage. Miros carries ten crossbow bolts.
```
^statblock