...
```ad-statblock
title: Aurelia
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/GGR/Aurelia.webp#token)
...
***Longsword.*** *Melee Weapon Attack:* `dice:1d20+15|noform|text(+15)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+8|noform|avg|text(12)` (`1d8 + 8`) slashing damage, or `dice:1d10+8|noform|avg|text(13)` (`1d10 + 8`) slashing damage when used with two hands, plus `dice:6d8|noform|avg|text(27)` (`6d8`) radiant damage.

***Leadership.*** Aurelia utters a few inspiring words to one creature she can see within 30 feet of her. If the creature can hear her, it can add a `dice:d10|noform|avg` (`d10`) to one attack roll or saving throw it makes before the start of Aurelia's next turn.

***Warleader's Helix (Recharge 5-6).*** *Ranged Spell Attack:* `dice:1d20+17|noform|text(+17)` to hit, range 60 ft., one creature. *Hit:* `dice:12d8|noform|avg|text(54)` (`12d8`) radiant damage, and Aurelia can choose another creature she can see within 10 feet of the target. The second creature regains `dice:6d8|noform|avg|text(27)` (`6d8`) hit points.
...
***Unyielding.*** When Aurelia is subjected to an effect that would move her, knock her [prone](2-Mechanics/CLI/rules/conditions.md#Prone), or both, she can use her reaction to be neither moved nor knocked [prone](2-Mechanics/CLI/rules/conditions.md#Prone).
...
***Frighten Foes (Costs 3 Actions).*** Aurelia targets up to five creatures she can see within 30 feet of her. Each target must succeed on a DC 25 Wisdom saving throw or be [frightened](2-Mechanics/CLI/rules/conditions.md#Frightened) of her until the end of her next turn. Any target within 5 feet of Aurelia has disadvantage on the saving throw.
```
^statblock