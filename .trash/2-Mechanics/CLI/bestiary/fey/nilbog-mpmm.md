...
***Nilbogism.*** Any creature that attempts to damage the nilbog must first succeed on a DC 12 Charisma saving throw or be [charmed](2-Mechanics/CLI/rules/conditions.md#Charmed) until the end of the creature's next turn. A creature [charmed](2-Mechanics/CLI/rules/conditions.md#Charmed) in this way must use its action praising the nilbog.
...
**At will**: [mage hand](2-Mechanics/CLI/spells/mage-hand.md), [Tasha's hideous laughter](2-Mechanics/CLI/spells/tashas-hideous-laughter.md)

## Actions

***Fool's Scepter.*** *Melee Weapon Attack:* `dice:1d20+4|noform|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d6+2|noform|avg|text(5)` (`1d6 + 2`) bludgeoning damage.

***Mocking Word.*** The nilbog targets one creature it can see within 60 feet of it. The target must succeed on a DC 12 Wisdom saving throw or take `dice:2d4|noform|avg|text(5)` (`2d4`) psychic damage and have disadvantage on its next attack roll before the end of its next turn.

## Bonus Actions

***Nimble Escape.*** The nilbog takes the [Disengage](2-Mechanics/CLI/rules/actions.md#Disengage) or [Hide](2-Mechanics/CLI/rules/actions.md#Hide) action.

## Reactions

***Reversal of Fortune.*** In response to another creature dealing damage to the nilbog, the nilbog reduces the damage to 0 and regains `dice:1d6|noform|avg|text(3)` (`1d6`) hit points.
```
^statblock
