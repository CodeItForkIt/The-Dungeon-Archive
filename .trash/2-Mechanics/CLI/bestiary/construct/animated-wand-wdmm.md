...
***Wand of Wonder.*** The animated wand can expend 1 of its charges and target a random creature with one of its [randomly determined effects](2-Mechanics/CLI/items/wand-of-wonder.md). Any such effect that would target the wand's user targets the wand instead. If reduced to 0 hit points, the wand crumbles into dust and is destroyed.

***Slam.*** *Melee Weapon Attack:* `dice:1d20+8|noform|text(+8)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+4|noform|avg|text(6)` (`1d4 + 4`) bludgeoning damage.
```
^statblock