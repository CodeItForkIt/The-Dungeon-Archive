...
***Magical Strike.*** *Melee Spell Attack:* `dice:1d20+8|noform|text(+8)` to hit, reach 10 ft., one target. *Hit:* `dice:5d6+5|noform|avg|text(22)` (`5d6 + 5`) poison damage.
...
The fog moves 10 feet away from the living spell at the start of each of its turns, rolling along the ground and through openings. The fog lasts for 10 minutes or until the living spell's [concentration](2-Mechanics/CLI/rules/conditions.md#Concentration) ends (as if concentrating on a spell).
```
^statblock