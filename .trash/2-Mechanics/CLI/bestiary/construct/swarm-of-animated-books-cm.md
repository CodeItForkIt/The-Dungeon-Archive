...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/CM/Swarm%20of%20Animated%20Books.webp#token)
...
***False Objects.*** If the swarm is motionless at the start of combat, it has advantage on its initiative roll. Moreover, if a creature hasn't observed the swarm move or act, that creature must succeed on a DC 15 Wisdom ([Perception](2-Mechanics/CLI/rules/skills.md#Perception)) check to discern that the swarm is animate.
...
***Book Club.*** *Melee Weapon Attack:* `dice:1d20+3|noform|text(+3)` to hit, reach 0 ft., one target in the swarm's space. *Hit:* `dice:2d4+1|noform|avg|text(6)` (`2d4 + 1`) bludgeoning damage, or `dice:1d4+1|noform|avg|text(3)` (`1d4 + 1`) bludgeoning damage if the swarm has half its hit points or fewer.
```
^statblock