...
***Claw.*** *Melee Weapon Attack:* `dice:1d20+3|noform|text(+3)` to hit, reach 5 ft., one target. *Hit:* `dice:2d4+1|noform|avg|text(6)` (`2d4 + 1`) slashing damage. If the target is a creature, it must succeed on a DC 11 Wisdom saving throw or be [frightened](2-Mechanics/CLI/rules/conditions.md#Frightened) until the end of the scarecrow's next turn.

***Terrifying Glare.*** The scarecrow targets one creature it can see within 30 feet of it. If the target can see the scarecrow, the target must succeed on a DC 11 Wisdom saving throw or be magically [frightened](2-Mechanics/CLI/rules/conditions.md#Frightened) until the end of the scarecrow's next turn. The [frightened](2-Mechanics/CLI/rules/conditions.md#Frightened) target is [paralyzed](2-Mechanics/CLI/rules/conditions.md#Paralyzed).
```
^statblock
