...
***Barbed Hide.*** At the start of each of its turns, the barbed devil deals `dice:1d10|noform|avg|text(5)` (`1d10`) piercing damage to any creature grappling it.
...
***Claw.*** *Melee Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, reach 5 ft., one target. *Hit:* `dice:1d6+3|noform|avg|text(6)` (`1d6 + 3`) piercing damage.

***Tail.*** *Melee Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+3|noform|avg|text(10)` (`2d6 + 3`) piercing damage.

***Hurl Flame.*** *Ranged Spell Attack:* `dice:1d20+5|noform|text(+5)` to hit, range 150 ft., one target. *Hit:* `dice:3d6|noform|avg|text(10)` (`3d6`) fire damage. If the target is a flammable object that isn't being worn or carried, it also catches fire.
```
^statblock