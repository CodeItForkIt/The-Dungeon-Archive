---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/coa
- monster/cr/12
- monster/size/medium
- monster/type/fiend
aliases: ["Slayer"]
---
# Slayer
*Source: Chains of Asmodeus p. 30*  

```ad-statblock
title: Slayer
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/CoA/Slayer.webp#token)
*Medium fiend, host's alignment*

- **Armor Class** 19 (natural armor)
- **Hit Points** 136 (`16d8 + 64`)
- **Speed** 40 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|20 (+5)|19 (+4)|18 (+4)|13 (+1)|14 (+2)|14 (+2)|

- **Proficiency Bonus** +4
- **Saving Throws** Dexterity +8, Intelligence +5
- **Skills** Acrobatics +8, Deception +6, Perception +6, Stealth +12
- **Senses** passive Perception 16
- **Damage Resistances** bludgeoning, piercing, slashing from nonmagical attacks that aren't silvered
- **Damage Immunities** poison
- **Languages** Languages of the host
- **Challenge** 12

## Traits

***Assassinate.*** During its first turn, the Slayer has advantage on attack rolls against any creature that hasn't taken a turn. Any hit the Slayer scores against a surprised creature is a critical hit.

***Devil's Sight.*** Magical darkness doesn't impede the Slayer's darkvision.

***Magic Resistance.*** The Slayer has advantage on saving throws against spells and other magical effects.

## Actions

***Multiattack.*** The Slayer makes four Claw attacks.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+9|noform|text(+9)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+5|noform|avg|text(12)` (`2d6 + 5`) slashing damage plus `dice:2d6|noform|avg|text(7)` (`2d6`) poison damage. The Slayer deals an extra `dice:4d6|noform|avg|text(14)` (`4d6`) slashing damage when it has advantage on the attack roll and hits the target.
```
^statblock