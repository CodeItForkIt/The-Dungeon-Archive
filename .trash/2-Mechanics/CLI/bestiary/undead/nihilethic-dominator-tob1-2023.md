...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/ToB1-2023/Nihilethic%20Dominator.webp#token)
...
***Claw (Undead Form Only).*** *Melee Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, reach 5 ft., one target. *Hit:* `dice:2d8+3|noform|avg|text(12)` (`2d8 + 3`) slashing damage. If the target is a creature, it must succeed on a DC 14 Strength saving throw or be knocked [prone](2-Mechanics/CLI/rules/conditions.md#Prone).

***Tendril of the Void (Void Ghost Form Only).*** *Melee Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, reach 10 ft., one creature. *Hit:* `dice:1d8+3|noform|avg|text(7)` (`1d8 + 3`) cold damage plus `dice:2d6|noform|avg|text(7)` (`2d6`) necrotic damage.
...
***Void Body.*** When the nihilethic dominator takes damage that isn't radiant, it can reduce the damage by `dice:1d12|noform|avg` (`1d12`).
```
^statblock
