...
***False Appearance.*** If the trapper is motionless on a floor, wall, or ceiling at the start of combat, it has advantage on its initiative roll. Moreover, if a creature hasn't observed the trapper move or act, that creature must succeed on a DC 18 Intelligence ([Investigation](2-Mechanics/CLI/rules/skills.md#Investigation)) check to discern that the trapper isn't part of the floor, wall, or ceiling.
...
***Smother.*** One Large or smaller creature within 10 feet of the trapper must succeed on a DC 13 Dexterity saving throw or be [grappled](2-Mechanics/CLI/rules/conditions.md#Grappled) (escape DC 14). Until the grapple ends, the target takes `dice:3d6+3|noform|avg|text(13)` (`3d6 + 3`) bludgeoning damage plus `dice:1d6|noform|avg|text(3)` (`1d6`) acid damage at the start of each of its turns. While [grappled](2-Mechanics/CLI/rules/conditions.md#Grappled) in this way, the target is [restrained](2-Mechanics/CLI/rules/conditions.md#Restrained), [blinded](2-Mechanics/CLI/rules/conditions.md#Blinded), and deprived of air. The trapper can smother only one creature at a time.
```
^statblock
