---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/skt
- monster/cr/1-8
- monster/size/medium
- monster/type/humanoid/elf
aliases: ["Sea Elf"]
---
# Sea Elf
*Source: Storm King's Thunder p. 70*  

```ad-statblock
title: Sea Elf
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/SKT/Sea%20Elf.webp#token)
*Medium humanoid (elf), Chaotic Good*

- **Armor Class** 11
- **Hit Points** 11 (`2d8 + 2`)
- **Speed** 10 ft., swim 40 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|10 (+0)|13 (+1)|12 (+1)|11 (+0)|11 (+0)|12 (+1)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** Perception +2
- **Senses** darkvision 60 ft., passive Perception 12
- **Languages** Common, Elvish
- **Challenge** 1/8

## Traits

***Fey Ancestry.*** The sea elf has advantage on saving throws against being [charmed](2-Mechanics/CLI/rules/conditions.md#Charmed), and magic can't put the sea elf to sleep.

***Amphibious.*** The sea elf can breathe air and water.

## Actions

***Spear.*** *Melee or Ranged Weapon Attack:* `dice:1d20+2|noform|text(+2)` to hit, reach 5 ft. or range 20/60 ft., one target. *Hit:* `dice:1d6|noform|avg|text(3)` (`1d6`) piercing damage, or `dice:1d8|noform|avg|text(4)` (`1d8`) piercing damage if used with two hands to make a melee attack.
```
^statblock