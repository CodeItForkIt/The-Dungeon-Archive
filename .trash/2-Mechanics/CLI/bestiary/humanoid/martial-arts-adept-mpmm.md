...
***Unarmed Strike.*** *Melee Weapon Attack:* `dice:1d20+5|noform|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+3|noform|avg|text(7)` (`1d8 + 3`) bludgeoning damage. Once per turn, the adept can cause one of the following additional effects (choose one or roll a `dice:d4|noform|avg` (`d4`)):

- **1–2 Knock Down..** The target must succeed on a DC 13 Dexterity saving throw or be knocked [prone](2-Mechanics/CLI/rules/conditions.md#Prone).  
