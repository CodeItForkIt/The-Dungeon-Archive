...
***Bites.*** *Melee Weapon Attack:* `dice:1d20+6|noform|text(+6)` to hit, reach 0 ft., one creature in the swarm's space. *Hit:* `dice:4d6|noform|avg|text(14)` (`4d6`) piercing damage, or `dice:2d6|noform|avg|text(7)` (`2d6`) piercing damage if the swarm has half of its hit points or fewer.
```
^statblock