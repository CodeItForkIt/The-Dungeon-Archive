...
| 8 | A howling wind blows through the party, and each party member must roll a `dice:d8\\|noform\\|avg` (`d8`). Anyone who rolls an 8 has a trinket or some other tiny, nonmagical item (such as a coin) stolen from them by weather spirits. |
^feywild-weather