---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/dsotdq
- monster/cr/
- monster/size/small
- monster/type/humanoid/kender
statblock: inline
aliases: ["Tem Temble (8th Level)"]
---
# [Tem Temble (8th Level)](Mechanics\bestiary\npc/tem-temble-8th-level-dsotdq.md)
*Source: Dragonlance: Shadow of the Dragon Queen p. 213*  

```statblock
"name": "Tem Temble (8th Level) (DSotDQ)"
"size": "Small"
"type": "humanoid"
"subtype": "kender"
"alignment": "Chaotic Good"
"ac": !!int "13"
"ac_class": "[leather armor](Mechanics/items/leather-armor.md)"
"hp": !!int "49"
"hit_dice": "9d6 + 18"
"stats":
- !!int "8"
- !!int "14"
- !!int "14"
- !!int "10"
- !!int "18"
- !!int "14"
"speed": "30 ft."
"saves":
  "Wisdom": !!int "7"
"skillsaves":
  "Medicine": !!int "7"
  "Sleight of Hand": !!int "5"
  "Stealth": !!int "5"
  "Insight": !!int "7"
  "Perception": !!int "7"
"condition_immunities": "[frightened](Mechanics/Rules/conditions.md#Frightened)"
"senses": "passive Perception 16"
"languages": "Common, Kenderspeak"
"traits":
- "desc": "Tem's spellcasting ability is Wisdom (spell save DC 15, +7 to spell attacks).\
    \ She has the following druid spells prepared:\n\nAt will: [druidcraft](Mechanics/spells/druidcraft.md),\
    \ [poison spray](Mechanics/spells/poison-spray.md), [mending](Mechanics/spells/mending.md)\n\
    \n1st level (3 slots): [healing word](Mechanics/spells/healing-word.md), [faerie\
    \ fire](Mechanics/spells/faerie-fire.md), [goodberry](Mechanics/spells/goodberry.md)\n\
    \n2nd level (1 slots): [enhance ability](Mechanics/spells/enhance-ability.md)"
  "name": "Spellcasting"
- "desc": "Tem is proficient with simple weapons and light armor."
  "name": "Bonus Proficiencies"
- "desc": "Tem adds her Wisdom modifier to the damage she deals with any cantrip."
  "name": "Potent Cantrips"
"actions":
- "desc": "Melee or Ranged Weapon Attack: +5 to hit, reach 5 ft. or range 40/160\
    \ ft., one target. Hit: 4 (1d4 + 2) piercing damage, or 4 (1d4 + 2) bludgeoning\
    \ damage if Tem used the hoopak's sling to make a ranged attack."
  "name": "Hoopak"
- "desc": "Tem launches an infuriating barrage of insults at a creature she can see\
    \ within 60 feet of her. If the target can hear Tem, it must succeed on a DC 15\
    \ Wisdom saving throw or have disadvantage on attack rolls until the end of its\
    \ next turn."
  "name": "Taunt"
"bonus_actions":
- "desc": "Tem takes the Disengage or Hide action."
  "name": "Elusive"
"source":
- "DSotDQ"
```
^statblock