---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/dsotdq
- monster/cr/
- monster/size/medium
- monster/type/humanoid/human
statblock: inline
aliases: ["Andir Valmakos (11th Level)"]
---
# [Andir Valmakos (11th Level)](Mechanics\bestiary\npc/andir-valmakos-11th-level-dsotdq.md)
*Source: Dragonlance: Shadow of the Dragon Queen p. 210*  

```statblock
"name": "Andir Valmakos (11th Level) (DSotDQ)"
"size": "Medium"
"type": "humanoid"
"subtype": "human"
"alignment": "Chaotic Good"
"ac": !!int "12"
"ac_class": "[leather armor](Mechanics/items/leather-armor.md)"
"hp": !!int "66"
"hit_dice": "12d8 + 12"
"stats":
- !!int "10"
- !!int "12"
- !!int "12"
- !!int "18"
- !!int "11"
- !!int "10"
"speed": "30 ft."
"saves":
  "Wisdom": !!int "4"
"skillsaves":
  "Investigation": !!int "8"
  "Religion": !!int "8"
  "History": !!int "8"
  "Arcana": !!int "8"
"senses": "passive Perception 10"
"languages": "Common, Draconic"
"traits":
- "desc": "Andir's spellcasting ability is Intelligence (spell save DC 16, +8 to\
    \ hit with spell attacks). He has the following wizard spells prepared:\n\nAt\
    \ will: [light](Mechanics/spells/light.md), [ray of frost](Mechanics/spells/ray-of-frost.md),\
    \ [mage hand](Mechanics/spells/mage-hand.md), [minor illusion](Mechanics/spells/minor-illusion.md)\n\
    \n1st level (4 slots): [thunderwave](Mechanics/spells/thunderwave.md), [longstrider](Mechanics/spells/longstrider.md),\
    \ [Tenser's floating disk](Mechanics/spells/tensers-floating-disk.md)\n\n2nd\
    \ level (3 slots): [knock](Mechanics/spells/knock.md), [invisibility](Mechanics/spells/invisibility.md)\n\
    \n3rd level (3 slots): [haste](Mechanics/spells/haste.md), [sending](Mechanics/spells/sending.md)"
  "name": "Spellcasting"
- "desc": "Andir is proficient with simple weapons and light armor."
  "name": "Bonus Proficiencies"
- "desc": "Andir adds his Intelligence modifier to the damage he deals with any cantrip."
  "name": "Potent Cantrips"
"actions":
- "desc": "Melee or Ranged Spell Attack: +4 to hit, reach 5 ft. or range 120 ft.,\
    \ one target. Hit: 7 (1d10 + 2) force damage."
  "name": "Arcane Burst"
- "desc": "Melee Weapon Attack: +4 to hit, reach 5 ft., one target. Hit: 3 (1d6)\
    \ bludgeoning damage, or 4 (1d8) bludgeoning damage if used with two hands."
  "name": "Quarterstaff"
"source":
- "DSotDQ"
```
^statblock