...
***Scrying Focus.*** A spellcaster can use the fractine as a substitute focus when casting the [scrying](Mechanics/spells/scrying.md) spell or similar magic, provided the spellcaster and the fractine are within 5 feet of each other.
...
The fractine can imprison only one creature at a time and can release that creature as a bonus action. If the fractine is reduced to 0 hit points, any creature in the fractine's demiplane is released instantly. A released creature reappears in an unoccupied space as close to the fractine (or where it died) as possible. A creature can leave the demiplane on its own by using magic that enables planar travel, such as the [plane shift](Mechanics/spells/plane-shift.md) spell.
